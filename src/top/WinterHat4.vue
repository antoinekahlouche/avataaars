<template>
	<g id="Top">
		<defs>
			...
			<filter x="-0.7%" y="-1.7%" width="101.4%" height="106.8%" filter-units="objectBoundingBox" :id="filter1">
		...
		<mask :id="mask1" fill="white">
			<use :xlink:href="selector(path5)" />
		</mask>
		<g id="Mask" />
		<g id="WinterHat4" transform="translate(-1.000000, 0.000000)">
			<g id="hat" stroke-width="1" fill-rule="evenodd" transform="translate(65.000000, 4.000000)">
				<mask :id="mask2" fill="white">
					<use :xlink:href="selector(path1)" />
				</mask>
				<use id="hat-mask" fill="#D8D8D8" :xlink:href="selector(path1)" />
				<HatColor :maskID="mask2" defaultColor="Red" />
				<mask :id="mask3" fill="white">
					<use :xlink:href="selector(path2)" />
				</mask>
				<use id="shadow" fill-opacity="0.24" fill="#000000" :xlink:href="selector(path2)" />
				<mask :id="mask4" fill="white">
					<use :xlink:href="selector(path3)" />
				</mask>
				<use id="light" fill-opacity="0.300000012" fill="#FFFFFF" :xlink:href="selector(path3)" />
			</g>
			<g id="hat-front">
				<use fill="black" fill-opacity="1" :filter="url(path4)" :xlink:href="selector(path4)" />
				<use fill="#F4F4F4" fill-rule="evenodd" :xlink:href="selector(path4)" />
			</g>
			<slot name="FacialHair" />
...
import HairColor from "../colors/Hair"
